---
title: "J6 · Faire parler les cartes : l'art de la visualisation — démonstration formateur"
subtitle: "Atelier IFORD × GDSG 2026 · Samedi 1er août 2026 · Référents : J.S. Alogo, M. Teda, R. Dzita"
...
Ce rapport est le **miroir exécuté** de `script_formateur_J6.R` : chaque section montre
...
# Insécurité alimentaire : aucun fichier dédié. Le matériel d'origine appelait
# un CSV FIES de la FAO qui n'a jamais été fourni. L'indicateur est dérivé de
# la variable de satisfaction alimentaire d'ECAM5 (voir module 2.2).
# Images Sentinel-2
...
# Aucune lecture ici : il n'existe pas de fichier d'insécurité alimentaire
# dans ce dossier. L'indicateur est construit au module 2.2 à partir de
# `ecam5` (variable de satisfaction alimentaire), déjà lu au point 4.
# Règle « une lecture, une fois » : on ne relit pas ecam5.dta plus bas.
...
## 2.2 Carte choroplèthe avancée — satisfaction alimentaire déclarée (ECAM5)

::: {.callout-warning}
## Le FIES n'existe pas dans ce dossier — ce qui le remplace, et ce que cela change

Le matériel d'origine cartographiait ici une **échelle FIES** (*Food Insecurity
Experience Scale*, FAO) lue dans un fichier CSV dédié. **Ce fichier n'a jamais
été fourni** : il n'est ni dans `datasets/`, ni ailleurs dans le dépôt. Les valeurs qui figuraient dans le code étaient en outre **écrites à la
main**, c'est-à-dire inventées. Une carte construite sur des nombres inventés
reste une belle carte : c'est exactement le danger rappelé au principe « une
carte fausse est plus dangereuse qu'un tableau faux ».

Le substitut retenu est la variable **`s09q13a` d'ECAM5**, la *satisfaction
alimentaire déclarée* du ménage.

**Ce n'est pas un FIES, et il ne faut pas le présenter comme tel.**

| | FIES (FAO) | `s09q13a` (ECAM5) |
|---|---|---|
| Construction | échelle **validée à 8 items**, calibrée par modèle de Rasch | **une seule question** de satisfaction déclarée |
| Grandeur mesurée | expérience vécue du manque de nourriture (12 derniers mois) | appréciation subjective de l'alimentation du ménage |
| Comparabilité | seuils internationaux (modérée / sévère) comparables entre pays | **aucun seuil international** ; le découpage est un choix local |
| Ce qu'on peut en dire | « prévalence de l'insécurité alimentaire sévère » | « part des ménages qui se déclarent non satisfaits de leur alimentation » |

La carte ci-dessous mesure donc **une insatisfaction déclarée**, pas une
prévalence d'insécurité alimentaire. Le titre, la légende et la source le
disent explicitement — c'est une exigence de méthode, pas une précaution
rédactionnelle.
:::

### Bloc défensif — que contient réellement `s09q13a` ?

Le dictionnaire d'ECAM5 ne garantit ni la présence ni le codage de cette
variable : le module ne suppose donc rien et commence par **imprimer la donnée
avant de la traiter** — présence de la colonne, libellé de la variable,
libellés des modalités, distribution. La règle de recodage est ensuite dérivée
des libellés observés, pas d'un ordre supposé des codes numériques.

```{r}
#| label: chunk-17a
# ── MODULE 2.2 (a) : inspection defensive de s09q13a ──────────────────
# A LIRE AVANT DE FAIRE CONFIANCE A LA CARTE. Ce bloc ne calcule rien :
# il montre ce que la variable contient, pour que le recodage du bloc
# suivant soit valide ou corrige AU PREMIER RENDU.
...
# haven::as_factor() n a alors rien a traduire. On passe par ce garde-fou.
...
if (ok_s09q13a) {
  ...
  if (is.null(lab_val)) {
    cat('  (aucun libelle de valeur : la variable est numerique nue)\n')
  } else {
    print(lab_val)
  }

  cat('\nDistribution, tous individus (NA compris) :\n')
  print(table(en_facteur(ecam5$s09q13a), useNA = 'ifany'))

  cat('\nDistribution, chefs de menage seulement (s01q3 == 1) :\n')
  print(table(en_facteur(ecam5$s09q13a[ecam5$s01q3 == 1]),
              useNA = 'ifany'))
} else {
  cat('ATTENTION : la variable attendue est absente. Le module 2.2 ne peut\n',
      'pas produire d indicateur. Verifier le dictionnaire d ECAM5.\n')
}
```

### Construction de l'indicateur

L'unité retenue est le **ménage**, pas l'individu : on restreint aux chefs de
ménage (`s01q3 == 1`). Ce choix n'est pas cosmétique — parcourir les lignes
d'individus surreprésenterait les grands ménages (biais de taille), et la part
d'insatisfaction obtenue ne répondrait pas à la même question.

L'agrégation régionale est **pondérée par `coefextr`**, le coefficient
d'extrapolation d'ECAM5. Sans les poids, on décrit l'échantillon enquêté ; avec
eux, on estime la population. Le code imprime les deux, côte à côte, pour que
l'écart soit visible.

Enfin, ECAM5 code **12 régions d'enquête** dans `s0q1` (Douala et Yaoundé y sont
distinguées), alors que la carte GADM n'en a que **10** : Douala et Yaoundé ne
sont pas des régions administratives. Le rattachement Douala → Littoral et
Yaoundé → Centre est une **décision de méthode**, faite avant l'agrégation et
non après, pour que la pondération reste correcte.

```{r}
#| label: chunk-17b
# ── MODULE 2.2 (b) : indicateur d insatisfaction alimentaire ponderé ───

# --- Regle de recodage, DERIVEE DES LIBELLES imprimes au bloc precedent ---
...
# satisfaits ; celles qui portent une marque de non-reponse sont mises a NA
# (non-reponse, pas "satisfait"). Si les libelles reels ne suivent pas cette
# forme, corriger les deux motifs ci-dessous — c est le seul endroit a changer.
...
sans_accents <- function(x) {
  chartr('àâäéèêëîïôöùûüç',
         'aaaeeeeiioouuuc', tolower(as.character(x)))
}
...
if (ok_s09q13a && !is.null(lab_val)) {
  ...
  recodage_valide <- length(codes_insat) > 0

} else {
  codes_hors <- integer(0); codes_insat <- integer(0)
  recodage_valide <- FALSE
  cat('Recodage impossible : pas de libelles de valeurs exploitables.\n',
      'Renseigner a la main codes_insat et codes_hors apres lecture du bloc 17a.\n')
}

if (recodage_valide) {

  # Unite = le menage : un enregistrement par chef de menage
  men_ecam <- ecam5[!is.na(ecam5$s01q3) & ecam5$s01q3 == 1, ]
  ...
  print(table(c('0 = satisfait', '1 = non satisfait')[insat + 1], useNA = 'ifany'))

  # --- Rattachement des 12 regions d enquete aux 10 regions administratives ---
  # Decision de methode : Douala -> Littoral, Yaounde -> Centre. Le test porte
  # sur le libelle, ce qui couvre aussi les intitules du type
  # "Centre (sans Yaounde)" ou "Littoral (sans Douala)".
  reg_lab <- as.character(en_facteur(men_ecam$s0q1))
  reg_adm <- ifelse(grepl('douala',  sans_accents(reg_lab)), 'Littoral',
             ifelse(grepl('yaounde', sans_accents(reg_lab)), 'Centre', reg_lab))

  cat('\nCorrespondance region d enquete -> region administrative :\n')
  print(unique(data.frame(enquete = reg_lab, administrative = reg_adm)))

  cle <- function(x) gsub('[^a-z]', '', sans_accents(x))

  isa_region <- data.frame(
      reg_adm = reg_adm,
      insat   = insat,
      poids   = as.numeric(men_ecam$coefextr)
    ) %>%
    filter(!is.na(insat), !is.na(poids), !is.na(reg_adm)) %>%
    group_by(reg_adm) %>%
    summarise(
      n_menages     = n(),
    ...
    mutate(cle_reg = cle(reg_adm))

  cat('\nPart de menages non satisfaits de leur alimentation, par region :\n')
  print(as.data.frame(isa_region))
  cat('\nEcart pondere / non pondere (points) — max :',
      round(max(abs(isa_region$pct_pondere - isa_region$pct_brut)), 2), '\n')

  # --- Jointure a la couche geographique, en partant de la couche ---
  rgn_isa <- rgn %>%
    mutate(cle_reg = cle(NAME_1)) %>%
    left_join(isa_region, by = 'cle_reg')

  cat('Regions de la carte sans valeur apres jointure :',
      sum(is.na(rgn_isa$pct_pondere)), 'sur', nrow(rgn_isa), '\n')
  if (any(is.na(rgn_isa$pct_pondere)))
    cat('  Non appariees :',
        paste(rgn_isa$NAME_1[is.na(rgn_isa$pct_pondere)], collapse = ', '), '\n')

} else {
  rgn_isa <- rgn %>% mutate(n_menages = NA_integer_,
                            pct_pondere = NA_real_, pct_brut = NA_real_)
}
```

```{r}
#| label: chunk-17
# ── MODULE 2.2 (c) : choroplethe avancee avec tmap ────────────────────
# Les polygones sans valeur restent GRIS : ils ne sont pas "a zero".

carte_isa_tmap <- tm_shape(lim) +
  tm_polygons(col = '#F5F5F0', border.col = '#AAAAAA', lwd = 0.5) +

tm_shape(rgn_isa) +
  tm_polygons(
    col           = 'pct_pondere',           # Variable à cartographier
    title         = 'Ménages non\nsatisfaits de leur\nalimentation (%)',
    palette       = '-RdYlGn',               # Rouge = fort, Vert = faible
    style         = 'jenks',                 # Classification naturelle (Jenks)
    n             = 5,                       # Nombre de classes
    colorNA       = 'grey80',                # Non mesuré ≠ zéro
    textNA        = 'Non renseigné',
    border.col    = 'white',
    border.alpha  = 0.8,
    legend.format = list(suffix = '%'),
    popup.vars    = c('Région' = 'NAME_1',
                      'Non satisfaits, pondéré (%)'     = 'pct_pondere',
                      'Non satisfaits, non pondéré (%)' = 'pct_brut',
                      'Ménages enquêtés'                = 'n_menages')
  ) +
...
# Mise en page générale
  tm_layout(
    title            = 'Satisfaction alimentaire déclarée des ménages',
    title.size       = 1.0,
    title.fontface   = 'bold',
    title.color      = '#1F4E79',
    legend.outside   = TRUE,
  ...
  tm_credits(
    paste('Source : ECAM5 2022 (INS Cameroun), variable s09q13a, pondere par coefextr.',
          'Question unique de satisfaction declaree — ce n est PAS l echelle FIES de la FAO.',
          sep = '\n'),
    size     = 0.5,
...
# Sauvegarder
tmap_save(carte_isa_tmap,
          filename = paste0(dir_out, 'carte_satisfaction_alimentaire_ecam5.png'),
...
**Ce que la carte code.** La couleur d'une région est la **part des ménages qui
ne se déclarent pas satisfaits de leur alimentation**, estimée sur les chefs de
ménage d'ECAM5 et pondérée par `coefextr`. Le rouge est fort, le vert faible.
Ce n'est ni la prévalence d'insécurité alimentaire modérée, ni la sévère, au
sens de la FAO : aucune des deux n'est calculable ici.

**Ce que les choix techniques font.** La discrétisation *Jenks* cherche les
ruptures naturelles de la distribution : elle flatte les contrastes réels mais
rend deux cartes de deux millésimes **non comparables**, puisque les bornes
changent avec les données. Une discrétisation en quantiles donnerait cinq
classes d'effectif égal — donc toujours des rouges, même si tout le pays était
satisfait. Le même tableau raconte deux histoires selon la règle choisie.
La palette est **divergente inversée** (`-RdYlGn`) : elle suppose un point
médian sémantique, ce que la variable n'a pas vraiment — à discuter en salle.

**Ce qui se lit.** À commenter au premier rendu, une fois les libellés connus :
tant que la sortie du bloc 17a n'a pas été relue, aucun chiffre de cette carte
ne doit être cité.

**Ce qui ne se lit pas.** La moyenne régionale écrase l'hétérogénéité interne
(erreur écologique : un ménage d'une région rouge n'est pas insatisfait) ;
Douala et Yaoundé ont été fondues dans le Littoral et le Centre, ce qui masque
le contraste capitale/reste-de-région ; l'incertitude d'échantillonnage n'est
pas figurée ; et la satisfaction déclarée dépend de la norme locale d'attente,
donc n'est pas strictement comparable d'une région à l'autre.
:::
...
# - carte_isa_tmap   : satisfaction alimentaire déclarée, ECAM5 s09q13a
#                      (objet tmap -> à convertir en ggplot pour patchwork)
...
panneau_patchwork <- (carte_mortalite | carte_conso) /
  ...
  plot_annotation(
    ...
    subtitle = 'Sources : DHS 2018, ECAM5 2022, Sentinel-2 2026',
...
tmap_save(carte_isa_tmap,
          filename = paste0(dir_out, 'carte_satisfaction_alimentaire.png'),
          width=20, height=22, dpi=300, units='cm')

tmap_save(carte_isa_tmap,
          filename = paste0(dir_out, 'carte_satisfaction_alimentaire.pdf'),
          width=20, height=22, units='cm')

# TMAP — Mode interactif HTML
tmap_mode('view')
carte_isa_interactive <- tm_shape(rgn_isa) +
  tm_polygons(col = 'pct_pondere', palette = '-RdYlGn',
              colorNA = 'grey80', popup.vars = TRUE) +
  tm_layout(title = 'Satisfaction alimentaire déclarée — ECAM5 2022')
tmap_save(carte_isa_interactive,
          filename = paste0(dir_out, 'carte_satisfaction_alimentaire_interactive.html'))
tmap_mode('plot')
