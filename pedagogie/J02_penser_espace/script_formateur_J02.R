...
## J02 -- PENSER L'ESPACE : OU, ET POURQUOI LA ?
## ----------------------------------------------------------------------------
## Atelier IFORD x GDSG 2026 -- Mardi 28 juillet 2026
## Referents : M. Teda, R. Dzita -- Support : R. Elandi
##
## CE FICHIER EST LE MIROIR DE demo_formateur_J02.qmd : meme code, meme ordre,
...
##
library(tidyverse)  # Charge dplyr, readr, ggplot2, tidyr, etc.
library(skimr)      # Resumes statistiques enrichis
...
pop <- read_delim(
  "datasets/CMR_population_v1_0_admin_level2.csv",
  delim = ";", locale = locale(encoding = "UTF-8"), show_col_types = FALSE
) |>
  clean_names()   # "Nom Colonne" -> "nom_colonne"

cat("Population :", nrow(pop), "lignes x", ncol(pop), "colonnes\n")
glimpse(pop)

# --- Menages par departement (meme source, separateur VIRGULE) --------
men <- read_csv(
  "datasets/CMR_household_v1_0_admin_level2.csv",
  locale = locale(encoding = "UTF-8"), show_col_types = FALSE
) |>
  clean_names()

cat("\nMenages :", nrow(men), "lignes x", ncol(men), "colonnes\n")
glimpse(men)
...
depts <- pop |>
  select(id, departement = names1,
         pop_total = total, pop_bas = lower, pop_haut = upper,
         incertitude = uncertainty) |>
  left_join(
    men |> select(id, menages = total),
    by = "id"
  ) |>
  mutate(
    taille_menage = round(pop_total / menages, 2),
    # largeur de l'intervalle, en % de l'estimation centrale
    marge_pct     = round((pop_haut - pop_bas) / pop_total * 100, 0)
  )

# --- Les 10 departements les plus peuples ------------------------------
depts |>
  arrange(desc(pop_total)) |>
  select(departement, pop_total, menages, taille_menage) |>
  slice_head(n = 10) |>
  print()
...
depts |>
  arrange(desc(marge_pct)) |>
  select(departement, pop_total, pop_bas, pop_haut, marge_pct) |>
  slice_head(n = 5) |>
  print()

cat("\nMarge d'incertitude mediane :",
    median(depts$marge_pct, na.rm = TRUE), "% de l'estimation\n")
...
names(depts)
cat("\nColonne de region ? ", "region" %in% names(depts), "\n")
cat("Colonne de coordonnees ? ",
    any(grepl("lat|lon", names(depts))), "\n")
...
p1 <- depts |>
  slice_max(pop_total, n = 15) |>
  mutate(departement = reorder(departement, pop_total)) |>
  ggplot(aes(x = departement, y = pop_total / 1e3, fill = pop_total)) +
  geom_col(show.legend = FALSE) +
  scale_fill_gradient(low = "#AED6F1", high = "#1F4E79") +
  coord_flip() +
  labs(
    title    = "Les 15 departements les plus peuples",
    subtitle = "Source : WorldPop, estimations modelisees",
    x = NULL, y = "Population (milliers)"
  ) +
...
p2 <- depts |>
  ggplot(aes(x = pop_total / 1e3, y = marge_pct)) +
  geom_point(color = "#E67E22", alpha = 0.8, size = 2) +
  geom_smooth(method = "loess", formula = y ~ x, se = FALSE,
              color = "#1F4E79", linewidth = 0.8) +
  scale_x_log10() +
  labs(
    title    = "Petits departements, estimations plus incertaines",
    subtitle = "Largeur de l'intervalle de credibilite, en % de l'estimation",
    x = "Population (milliers, echelle log)", y = "Marge (%)"
  ) +
  theme_minimal(base_size = 11)

p1 + p2 +
  plot_annotation(
    title   = "DEMOGRAPHIE DU CAMEROUN PAR DEPARTEMENT",
    caption = "Formation IFORD x GDSG | Donnees : WorldPop CMR admin L2"
  )

ggsave("outputs/cartes/demographie_departements.png", dpi = 300,
       width = 14, height = 7, units = "in")

# --- Graphique 3 : taille moyenne des menages --------------------------
depts |>
  ggplot(aes(x = taille_menage)) +
  geom_histogram(binwidth = 0.25, fill = "#2E75B6", color = "white") +
  geom_vline(xintercept = median(depts$taille_menage, na.rm = TRUE),
             linetype = "dashed", color = "#E67E22", linewidth = 0.9) +
  labs(
    title    = "Taille moyenne des menages par departement",
    subtitle = "Trait orange : mediane nationale",
    x = "Personnes par menage", y = "Nombre de departements"
  ) +
  theme_minimal(base_size = 11)
...
## ========================================================================
library(sf)        # Simple Features pour données vectorielles
...
cmr0 <- st_read("datasets/gadm41_CMR_0.shp", quiet = TRUE)  # Pays
cmr1 <- st_read("datasets/gadm41_CMR_1.shp", quiet = TRUE)  # Régions
cmr2 <- st_read("datasets/gadm41_CMR_2.shp", quiet = TRUE)  # Départements

# --- Inspecter la structure d'un objet sf -----------------------------
class(cmr1)       # "sf" "data.frame" — c'est un data frame avec géométrie !
st_crs(cmr1)      # Affiche le CRS : EPSG:4326 (WGS84)
st_bbox(cmr1)     # Emprise spatiale : xmin, ymin, xmax, ymax
names(cmr1)       # Colonnes : NAME_1, GID_1... et geometry

# Nombre de régions
cat("Nombre de régions :", nrow(cmr1), "\n")  # 10 régions
...
cmr1_utm <- st_transform(cmr1, 32633)
cmr1_utm$superficie_km2 <- as.numeric(st_area(cmr1_utm)) / 1e6
cmr1_utm$superficie_km2 <- round(cmr1_utm$superficie_km2, 0)

# Centroïdes des régions (utiles pour placer des étiquettes)
centroides <- st_centroid(cmr1_utm) |>
  bind_cols(as.data.frame(st_coordinates(st_centroid(cmr1_utm)))) |>
  rename(lon_utm = X, lat_utm = Y)

# Tableau des régions et superficies
cmr1_utm |>
  st_drop_geometry() |>  # Enlever la géométrie pour affichage tabulaire
  select(NAME_1, superficie_km2) |>
  arrange(desc(superficie_km2))

# --- Vérification des géométries ----------------------------------------
sum(!st_is_valid(cmr1))  # 0 = toutes les géométries sont valides
...
grappes_shp <- st_read("datasets/CMGE71FL.shp", quiet = TRUE) |>
  clean_names()

st_crs(grappes_shp)$input          # CRS : EPSG:4326 (WGS84)
cat("Grappes EDS :", nrow(grappes_shp), "\n")
table(grappes_shp$urban_rura)      # repartition urbain / rural

# --- 2. Les attributs : lire le CSV de covariables ---------------------
covar_eds <- read_csv("datasets/CMGC72FL.csv", show_col_types = FALSE) |>
  clean_names()

cat("Covariables :", nrow(covar_eds), "grappes x",
    ncol(covar_eds), "variables\n")
# Ce fichier n'a AUCUNE colonne de coordonnees : c'est le shapefile qui
# porte la position. Verifions-le :
cat("Colonnes de coordonnees dans le CSV ? ",
    any(grepl("^lat|^lon", names(covar_eds))), "\n")

# --- 3. Recoller geometrie et attributs, par DHSCLUST ------------------
grappes_sf <- grappes_shp |>
  select(dhsclust, urban_rura, adm1name, alt_dem) |>
  left_join(covar_eds, by = "dhsclust")

class(grappes_sf)   # "sf" "data.frame" : un tableau avec une geometrie

# --- 4. Jointure SPATIALE : dans quelle region tombe chaque grappe ? ---
# La cle n'est plus un identifiant, c'est la position elle-meme.
grappes_avec_region <- st_join(
  grappes_sf,
  cmr1[, c("NAME_1", "GID_1")],
  join = st_within                 # la grappe est DANS le polygone
)

# Nombre de grappes par region et par milieu
grappes_avec_region |>
  st_drop_geometry() |>
  count(NAME_1, urban_rura) |>
  pivot_wider(names_from = urban_rura, values_from = n, values_fill = 0) |>
  print()

# --- Premiere carte rapide avec tmap -----------------------------------
tmap_mode("plot")  # Mode statique ("view" pour interactif)

# Syntaxe tmap 4 : le remplissage est 'fill', le contour est 'col'.
tm_shape(cmr1) +
  tm_polygons(fill = "#AED6F1", col = "#1F4E79", lwd = 0.8) +
tm_shape(grappes_sf) +
  tm_dots(
    fill       = "urban_rura",
    fill.scale = tm_scale_categorical(values = c("U" = "#E67E22",
                                                 "R" = "#27AE60")),
    fill.legend = tm_legend(title = "Milieu"),
    size = 0.4
  ) +
  tm_title("Grappes EDS Cameroun 2018") +
...
## télédétection.
##
library(terra)  # Paquet moderne pour les rasters
...
rgb <- rast("datasets/2026-06-26-00_00_2026-06-26-23_59_Sentinel-2_L2A_True_color.tiff")

# --- Explorer les propriétés du raster ----------------------------------
b08                  # Affiche : taille, résolution, CRS, valeurs min/max
crs(b08)             # Vérifier le CRS
res(b08)             # Résolution en degrés ou mètres
ext(b08)             # Emprise spatiale
nlyr(b08)            # Nombre de couches (1 pour B08)

# --- Vérification d'alignement : les rasters doivent avoir le même CRS --
identical(crs(b08), crs(b11))  # TRUE si même CRS
...
global(ndmi, fun = c("mean", "sd", "min", "max"), na.rm = TRUE)

# --- Visualisation rapide -----------------------------------------------
plot(rgb, main = "Cameroun — Sentinel-2 True Color (26 juin 2026)")
plot(ndmi, main = "Indice NDMI — Humidité de la végétation",
     col = colorRampPalette(c("#E74C3C", "#F9E79F", "#1ABC9C"))(100))

# --- Rogner sur une région d'intérêt : masquer sur l'emprise GADM ------
# Reprojeter le shapefile dans le CRS du raster si nécessaire
cmr1_rast_crs <- st_transform(cmr1, crs(ndmi))

# Rogner et masquer
ndmi_cmr <- crop(ndmi, vect(cmr1_rast_crs))
ndmi_cmr <- mask(ndmi_cmr, vect(cmr1_rast_crs))

plot(ndmi_cmr, main = "NDMI Cameroun — Sentinel-2 26 juin 2026")

# --- Sauvegarder le résultat -------------------------------------------
writeRaster(ndmi_cmr, "outputs/processed/ndmi_cameroun_2026.tif",
            overwrite = TRUE)
...
cat("Table population :", head(depts$departement, 4), "\n")
cat("GADM NAME_2      :", head(cmr2$NAME_2, 4), "\n\n")

# Jointure naive, sur le nom brut :
essai_naif <- cmr2 |>
  st_drop_geometry() |>
  left_join(depts, by = c("NAME_2" = "departement"))
...
normaliser <- function(x) {
  x |>
    stringi::stri_trans_general("Latin-ASCII") |>   # Benoue <- Benoue
    tolower() |>
...
tibble(
  ...
  population = c("Benoue", "Nde", "Nyong et Soo",  "Koupe et Manengouba")
) |>
  mutate(cle_gadm = normaliser(gadm), cle_pop = normaliser(population),
         concorde = cle_gadm == cle_pop) |>
  print()

# --- Jointure attributaire, sur la cle normalisee ---------------------
cmr2_pop <- cmr2 |>
  mutate(cle = normaliser(NAME_2)) |>
  left_join(depts |> mutate(cle = normaliser(departement)), by = "cle") |>
  mutate(
    # Projeter en UTM 32N AVANT de mesurer : une surface calculee en
    # degres n'a aucun sens (cf. regle d'or, section 3.2).
    superficie_km2 = as.numeric(st_area(st_transform(geometry, 32632))) / 1e6,
    densite_pop    = round(pop_total / superficie_km2, 1)
  )
...
cmr2_pop |>
  ...
  summarise(
    n_departements = n(),
    pop_totale     = sum(pop_total, na.rm = TRUE),
    densite_moy    = round(mean(densite_pop, na.rm = TRUE), 1),
    .groups = "drop"
  ) |>
...
cmr2_pop |>
  ...
  select(NAME_2, NAME_1, pop_total, superficie_km2, densite_pop) |>
  slice_head(n = 5) |>
  print()
...
tmap_mode("plot")  # Mode statique pour export PDF/PNG haute résolution

# --- Carte 1 : Densité de population par département ------------------
carte_densite <- tm_shape(cmr2_pop) +
  tm_polygons(
    fill        = "densite_pop",
    ...
    fill.legend = tm_legend(title = "Hab./km²"),
    col         = "white",     # couleur des contours
    col_alpha   = 0.5
  ) +
tm_shape(cmr1) +
  tm_borders(col = "#1F4E79", lwd = 1.5) +  # Contours régions plus marqués
tm_shape(grappes_sf) +
  tm_dots(
    fill        = "urban_rura",
    fill.scale  = tm_scale_categorical(values = c("U" = "#E67E22",
                                                  "R" = "#2ECC71")),
    fill.legend = tm_legend(title = "Milieu EDS"),
    size = 0.25
  ) +
  tm_title("Densité de population — Cameroun",
           size = 1.2, color = "#1F4E79") +
  tm_compass(position = c("right", "top"), size = 1.5) +
  tm_scalebar(position = c("left", "bottom")) +
  tm_credits(
    "Sources : GADM 4.1, WorldPop 2022, EDS Cameroun 2018\nFormation INS — 2026",
...
cmr1_palu <- cmr1 |>
  left_join(paludisme_region, by = "NAME_1")

carte_palu <- tm_shape(cmr1_palu) +
  tm_polygons(
    fill        = "prevalence_moy",
    fill.scale  = tm_scale_intervals(style = "quantile", n = 5,
                                     values = "brewer.yl_or_br"),
    fill.legend = tm_legend(title = "Prévalence (%)"),
    col = "white"
  ) +
  tm_text("NAME_1", size = 0.55) +
  tm_title("Paludisme chez l'enfant — EDS Cameroun 2018") +
  tm_credits("Source : covariables géospatiales DHS (CMGC72FL)",
             position = c("left", "bottom"), size = 0.6) +
  tm_layout(frame = FALSE, legend.outside = TRUE)
...
p_palu <- ggplot(paludisme_region, aes(x = aridite_moy, y = prevalence_moy)) +
  geom_point(aes(size = n_grappes), color = "#1F4E79", alpha = 0.7)

# ggrepel ecarte les etiquettes qui se chevauchent ; sinon repli simple.
p_palu <- if (requireNamespace("ggrepel", quietly = TRUE)) {
  p_palu + ggrepel::geom_text_repel(aes(label = NAME_1), size = 3)
} else {
  p_palu + geom_text(aes(label = NAME_1), size = 3, vjust = -0.9)
}

p_palu +
  labs(
    title    = "Le paludisme suit l'humidité — mais pas partout",
    subtitle = "Chaque point est une région ; la taille indique le nombre de grappes",
    x = "Indice d'aridité (plus élevé = plus humide)",
    y = "Prévalence du paludisme (%)",
    size = "Grappes"
  ) +
  theme_minimal(base_size = 11)
...
resume_national <- cmr2_pop |>
  st_drop_geometry() |>
  summarise(
    pop_totale        = sum(pop_total, na.rm = TRUE),
    n_departements    = n(),
    densite_moy       = round(mean(densite_pop, na.rm = TRUE), 0),
    densite_max       = round(max(densite_pop, na.rm = TRUE), 0),
...
resume_national |>
  gt() |>
  tab_header(
    title    = "Indicateurs demographiques -- Cameroun",
    subtitle = "Sources : WorldPop (admin 2), EDS Cameroun 2018"
  ) |>
  cols_label(
    pop_totale        = "Population",
    n_departements    = "Departements",
    densite_moy       = "Densite moyenne",
    densite_max       = "Densite maximale",
    taille_menage_moy = "Taille des menages"
  ) |>
  fmt_number(columns = pop_totale, suffixing = TRUE) |>
  tab_style(
    style = list(cell_fill(color = "#1F4E79"), cell_text(color = "white")),
    locations = cells_column_labels()
  )

# --- Carte finale annotee pour rapport ---------------------------------
carte_finale <- tm_shape(cmr2_pop) +
  tm_polygons(
    fill        = "densite_pop",
    fill.scale  = tm_scale_intervals(style = "jenks", n = 5,
                                     values = "brewer.yl_or_rd"),
    fill.legend = tm_legend(title = "Densite (hab/km2)")
  ) +
tm_shape(cmr1) +
  tm_borders(col = "#1F4E79", lwd = 2) +
  tm_text("NAME_1", size = 0.55, col = "#1F4E79", fontface = "bold") +
tm_shape(grappes_sf) +
  tm_dots(
    fill       = "urban_rura",
    fill.scale = tm_scale_categorical(values = c("U" = "#E67E22",
  ...
  tm_title(paste0("Cameroun -- Portrait spatial\n",
                  "Pop. totale : ",
                  format(round(resume_national$pop_totale / 1e6, 1)),
                  " millions")) +
  tm_compass(type = "arrow", position = c("right", "top")) +
  tm_scalebar(position = c("left", "bottom")) +
  tm_credits(
    "Formation IFORD x GDSG | Sources : GADM 4.1, WorldPop, EDS CMR 2018, Sentinel-2",
    size = 0.55, position = c("left", "bottom")
  ) +
  tm_layout(frame = FALSE, bg.color = "#FDFEFE", legend.outside = TRUE)

carte_finale

tmap_save(carte_finale, "outputs/cartes/portrait_spatial_cameroun.png",
          dpi = 300, width = 12, height = 10, units = "in")
cat("Carte de synthese sauvegardee.\n")
