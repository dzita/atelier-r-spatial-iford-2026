# Ménage du poste — commandes `del` et `ren` à passer à la main

> Ces commandes sont à passer manuellement, dans une invite de commandes
> Windows (`cmd.exe`), après lecture de la raison technique de chaque ligne :
> elles suppriment ou renomment des fichiers, l'opération n'est pas réversible
> (règle §6.7 du référentiel).
>
> **Avant toute chose : une branche Git.** Ces commandes détruisent des
> fichiers. Créer `git switch -c menage-poste` (ou un commit propre de l'état
> actuel) avant de les passer, comme le prescrit §6.7.
>
...
> *non trouvés*, sans commande associée : aucun `del` n'est fourni pour un
> fichier qui n'a pas été constaté sur disque.
...
>
> Supprimer les anciens fichiers avant que les nouveaux `*_J08.*` existent
> laisse un dossier-jour vide et plus rien à comparer.
>
...
> ```
>
> Les `del` ci-dessous ne se passent **que si** cette commande liste bien
> `demo_formateur_J08.qmd`, `script_formateur_J08.R`, `script_etudiant_J08.R` et
> `script_etudiant_J08_corrige.R`. Une liste vide ou partielle signifie que la
> journée n'est pas complète : ne rien supprimer.
...
> ```
>
> Les `del` ne se passent que si les quatre fichiers `*_J09.*` sont bien
> listés.
...
numérotation. **À conserver** : ils ne portent pas de numéro dans leur nom, ils
ne sont donc pas concernés par la règle §0.4. Seul leur *contenu* est mis à
jour.
...
Ce fichier peut être supprimé **immédiatement** : sa suppression ne dépend
d'aucun contrôle préalable.
...
Le suivi d'avancement signale trois autres catégories de fichiers versionnés
à tort. **Elles ont été recherchées sur tout le dépôt :**
...
## 5. `all_data\` — deux renommages, sans dépendance
...
**`CMFW71FL.SAV` ne se renomme PAS.** C'est le fichier *enquêteurs* du DHS
...
1. Branche Git (`git switch -c menage-poste`).
...
3. **§3** — le `del` du `.Rhistory` et l'ajout au `.gitignore` (aucune
   dépendance).
4. **§1** puis **§2** — vérifier par `dir /b` que les fichiers `*_J08.*` et
   `*_J09.*` existent, **puis seulement** passer les `del`.
5. Relancer `source("outils/distribuer_donnees.R")` et lire le bilan `cat()` de
   fin : il nomme chaque fichier introuvable et chaque copie en échec.
